...
Prices are data, not literals: `prices.json` (embedded via `include_str!`, schema 1, keyed provider → model id) is parsed into a validated `PriceTable`. First-party constructors (`anthropic`, `openai`, `openai_responses`, `google`, `xai`, `groq`, `deepseek`, `mistral`, `zai`, `minimax`, `qwen`, `meta`) set `cost` via a private `priced()` lookup at construction; the named presets inherit it through `..Self::anthropic(..)` etc. Gateways/custom endpoints never look up (`None`). The lookup reads a process-wide resolved table (`OnceLock<RwLock<Layers>>`): precedence explicit `config.cost` > user layer (`PriceTable::install_override`, or `YOAGENT_PRICES` read once at first use; a bad file warns and is ignored) > fetched layer (`install_fetched`, which warns on every disagreement with built-in) > built-in, each layer replacing whole entries per `(provider, id)`, resolved at construction time. `ModelConfig::with_prices(&table)` re-resolves explicitly (never clears, applies to gateways); `ModelConfig::reprice()` repeats the constructor lookup (can clear; only for configs a first-party constructor priced — private `#[serde(skip)] list_priced` marker). `PRICED_PROVIDERS` is the one list of providers constructors look up (`priced()` debug-asserts it); `install_override` returns `Vec<PriceChange>` and warns on entries outside it, dropped tiers, or newly-unset cache rates. Live sources (`prices/fetch.rs`): `PriceTable::fetch(&PriceSource)` — `ModelsDev`/`ModelsDevAt` (mapped; inexpressible models skipped and reported via `fetch_with_report`, `alibaba`→`qwen`, `opencode`→`opencode-zen`), `YoagentMain` (our file on GitHub `main`), `Url` — and `fetch_cached(source, path, max_age, max_stale)` (falls back to a bounded stale cache then built-in; `CachedPrices` carries `age` and `error`); `install_fetched_with(table, FetchPolicy::AddOnly)` only fills gaps; never fetched implicitly; remote/cached tables parse leniently, hand-written ones strictly (`PriceError::NewerFormat`). Tests that mutate the global layers live in their own test binaries (`price_override_test`, `price_env_test`, `price_env_bad_test`, `price_fetch_test`). `tests/preset_prices_test.rs` pins every preset's full `CostConfig`; `tests/price_audit.rs` (`--ignored`) diffs every entry against models.dev. Change a price by editing the JSON entry (and its `verified`/`source`), never by adding a literal.
